...
namespace DatabaseWrapper.SqlServer
{
    ...
    public class DatabaseClient : IDisposable
    {
        ...
        public DatabaseClient(DatabaseSettings settings)
        {
            ...
            LogResults = settings.LogResults;
        ...
        public DataTable Select(string tableName, int? indexStart, int? maxResults, List<string> returnFields, Expression filter, ResultOrder[] resultOrder = null)
        {
            if (string.IsNullOrEmpty(tableName)) throw new ArgumentNullException(nameof(tableName));
        ...
        /// <param name="resultOrder">Specify on which columns and in which direction results should be ordered.</param>
        /// <returns>A DataTable containing the results.</returns>
        public DataTable Select(string tableName, List<string> returnFields = null, Expression filter = null, ResultOrder[] resultOrder = null)
        {
            return Select(tableName, null, null, returnFields, filter, resultOrder);
        }
        ...
        /// </summary>
        /// <param name="tableName">The table in which you wish to INSERT.</param>
        /// <param name="dataClass">the data that gets set though</param>
        /// <returns>A DataTable containing the results.</returns>
        public DataTable Insert<T>(string tableName, T dataClass)
        {
            return Insert(tableName, Helper.ClassToDictionary(dataClass));
        }
        ...
        /// </summary>
        /// <param name="tableName">The table in which you wish to INSERT.</param>
        /// <param name="objects">List of dictionaries containing key-value pairs for the rows you wish to INSERT.</param>
        public void InsertMultiple<T>(string tableName, List<T> objects)
        {
            List<Dictionary<string, object>> keyValuePairList = new List<Dictionary<string, object>>();
            foreach (T obj in objects)
            {
                keyValuePairList.Add(Helper.ClassToDictionary(obj));
            }
            InsertMultiple(tableName, keyValuePairList);
        }
        ...
        /// <summary>
        /// Upsert into the data table based on the filter
        /// </summary>
        /// <param name="table"></param>
        /// <param name="obj"></param>
        /// <param name="filter"></param>
        /// <returns></returns>
        public DataTable Upsert(string table, object obj, Expression filter)
        {
            if (Exists(table, filter))
            {
                return Update(table, Helper.ClassToDictionary(obj), filter);
            }
            else
            {
                return Insert(table, Helper.ClassToDictionary(obj));
            }
        }
